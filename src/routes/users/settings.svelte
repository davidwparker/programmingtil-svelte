...
<script>
  ...
  async function handleSubmitPassword() {
    submittingPw = true;
    errors = [];
    const { response, json } = await api.post($session.API_ENDPOINT, 'users/password', {
  ...
  <div class="flex flex-col items-center justify-center">
    <div class="max-w-md w-full">
      ...
      <form on:submit|preventDefault={handleSubmit}>
        ...
        <div class="mt-6">
          <SubmitButton {submitting} full icon={UiLockSolid}>Update</SubmitButton>
        </div>
      </form>
    </div>
    <div class="max-w-md w-full mt-6">
      <form on:submit|preventDefault={handleSubmitPassword}>
        ...
        <SubmitButton {submittingPw} full>Get Password Change Link</SubmitButton>
      </form>
    </div>
  </div>
</div>
